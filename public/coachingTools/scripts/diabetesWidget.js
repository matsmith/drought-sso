...
define('coachingDiabetesWidget', [
	...
	function DiabetesViewModel(data) {
	...
	$.widget("wnp.coachingDiabetesWidget", $.wnp.baseWidget, {
		_widgetClass: "coachingDiabetesWidget",
		options: {
			initialView: "current",
			daysToInclude: 93,
			loadingSpinner: true
		...
		_createDom: function() {
			this._log("Diabetes._create called", arguments);
			this._setDom(Templates.diabetesWidget(this.data.textContent));
			this._initGraph(this.data);
			this.vm = new DiabetesViewModel(this.data);

			ko.applyBindings(this.vm, this.element[0]);
			this.tickLabelColor = colorUtils.getPrivateLabelColor(
				4, "diabetesSkills", this.companyConfig);
		...
		_updateViewWithLastScores: function(button) {
			...
			var form = $button.parents('[data-wnpcoachingwidget]').find(".diabetesToolForm");
		...
		_destroy: function() {
			this._log("diabetes._destroy called", arguments);
		...
		_fetchData: function() {
			...
			var bundlePromise = this.services.resourceBundle.getResourceBundles(["common","coachingTools","diabetesCoachingTool"]);
			var feedbackPromise = this.services.toolFeedback.getFeedback("diabetesCoachingTool");
		...
		_getNewTargetedContent: function() {
			...
			feedbackPromise.done(function(response) {
				...
				self._log("GET DiabetesSkills Feedback failed",arguments);
			}).always(function() {
				self.element.find('.saveButton').removeClass("disabled");
				self.vm.submissionInProgress(false);
			});
		},
		_initialize: function() {
			this._log("diabetessWidget._initialize called", arguments);
		...
		_initGraph: function(data) {
			...
			this.data.dataSeries.labels = [
				this.data.textContent.diabetesCoachingTool.MedicalLabel,
				this.data.textContent.diabetesCoachingTool.LifestyleLabel,
				this.data.textContent.diabetesCoachingTool.EmotionalLabel
			];
		...
		_persistNewScores: function() {
			...
			var newData = [{
					...
					DataSourceName: "diabetesSkillsTool",
			...
			postPromise.done(function(response) {
				self._log("POST new diabetes scores succeeded", arguments);
				...
				self._log("POST new diabetes scores failed", arguments);
				self.element.find('.saveButton').removeClass("disabled");
				self.vm.submissionInProgress(false);
	...
	return function CoachingDiabetesWidget(target, options) {
		return $(target).coachingDiabetesWidget(options).data("wnpCoachingDiabetesWidget");
	};
